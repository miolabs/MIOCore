...
public class XMLSerialization: NSObject, XMLParserDelegate {
    ...
    ///
    /// > Empty (self-closing) elements, ones with no value inside them such as `<ReferenceDocument/>`,
    /// > carry no text, so they are omitted from the result: their key is absent rather than present
    /// > with an empty value.
    ///
    /// - Parameters:
